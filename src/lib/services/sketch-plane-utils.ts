...
/**
 * Build123d uses Z-up, Three.js uses Y-up.
 * CAD (X, Y, Z) -> Three.js (X, Z, -Y)
 *
 * Sketch plane mappings (in Three.js world coords):
 * - XY (CAD: Z=0 plane) -> Three.js: Y=0 ground plane
 *   Normal: (0, 1, 0), U: (1, 0, 0), V: (0, 0, -1)
 * - XZ (CAD: Y=0 plane) -> Three.js: Z=0 plane
 *   Normal: (0, 0, -1), U: (1, 0, 0), V: (0, 1, 0)
 * - YZ (CAD: X=0 plane) -> Three.js: X=0 plane
...
export function getSketchPlaneInfo(
  ...
  // Convert CAD origin to Three.js
  ...
  switch (plane) {
    case 'XY':
      // CAD XY plane (Z=0) -> Three.js ground plane (Y=0)
      normal = new THREE.Vector3(0, 1, 0);
      u = new THREE.Vector3(1, 0, 0);
      v = new THREE.Vector3(0, 0, -1);
      break;
    case 'XZ':
      // CAD XZ plane (Y=0) -> Three.js Z=0 plane
      normal = new THREE.Vector3(0, 0, 1);
      u = new THREE.Vector3(1, 0, 0);
      v = new THREE.Vector3(0, 1, 0);
      break;
    case 'YZ':
      // CAD YZ plane (X=0) -> Three.js X=0 plane
...
/**
 * Compute the CAD origin offset for an offset datum plane.
 */
