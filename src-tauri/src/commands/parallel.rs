...
use crate::ai::capabilities::{context_bucket_for_model, prompt_tier_for_bucket};
use crate::ai::cost;
...
- For assemblies, include explicit alignment constraints between parts (e.g. "part A base flush with part B top; centered on footprint").
- If multiple IDENTICAL parts are needed, output ONE part and use `instances: [[x,y,z], ...]` for the additional placements (position is the primary placement).
...
- All parts must be modeled centered on XY, bottom at Z=0 (local origin). Assembly positions parts using `position`.
- Do NOT decompose decorative features, fillets, or chamfers into separate parts
- Do NOT include these words/phrases in any part `description`: Build sequence, Extrude, subtract, intersect, union, shell, boolean, cut

Keep your response as short as possible. For single mode, return ONLY {"mode":"single"} with no other text.
You MUST respond with ONLY valid JSON. No prose, no explanation, no markdown. Just the JSON object."#;
...
fn build_part_prompt(
    ...
    format!(
        ...
        Assembly placement rule: build this part at the local origin (centered XY, bottom at Z=0).\n\
        Do NOT translate or rotate the entire part for assembly. The assembly will place it using `position`.\n\
        If the plan specifies multiple instances of this part, still generate only ONE instance here.\n\
        Use sibling dimensions only for interface sizing, NOT for shifting the part.\n\n\
        {}\n\n\
...
fn assemble_parts(parts: &[(String, String, [f64; 3], Vec<[f64; 3]>)]) -> Result<String, String> {
    ...
    for (name, code, _pos, _instances) in parts {
    ...
    for (name, _code, pos, instances) in parts {
        let var_name = format!("part_{}", name);
        assembled.push_str(&format!(
            "    Pos({}, {}, {}) * {},\n",
...
fn assembly_contract_issues(
    code: &str,
    parts: &[(String, String, [f64; 3], Vec<[f64; 3]>)],
) -> Vec<String> {
    let mut issues = Vec::new();
    for (name, _code, _pos, _instances) in parts {
        let var_name = format!("part_{}", name);
...
async fn build_system_prompt_with_retrieval(
    ...
    on_event: &Channel<MultiPartEvent>,
    prompt_tier: prompts::PromptTier,
) -> (String, retrieval::RetrievalResult) {
    let base = prompts::build_system_prompt_for_tier(
    ...
    let _ = on_event.send(MultiPartEvent::RetrievalStatus {
        message: if retrieval_result.items.is_empty() {
            "No retrieval snippets matched; using base system prompt.".to_string()
        } else {
            format!(
...
async fn run_generation_pipeline(
    ...
    let is_deepseek = config.ai_provider == "deepseek";
    let planner_user_message = if is_deepseek {
        format!(
            "{}\n\nRespond with ONLY the JSON object starting with {{. No other text.",
            enhanced_message
        )
    } else {
    ...
    for attempt in 1..=MAX_PLANNER_PARSE_ATTEMPTS {
        ...
        let mut planner_messages = if attempt == 1 {
            vec![
                ChatMessage {
                    role: "system".to_string(),
                ...
                ChatMessage {
                    role: "user".to_string(),
                    content: planner_user_message.clone(),
                },
            ]
        } else {
            ...
            let retry_instruction = if is_deepseek {
                format!(
                    "{}\n\nRespond with ONLY the JSON object starting with {{. No other text.",
                    retry_instruction
                )
        ...
        if is_deepseek {
            planner_messages.push(ChatMessage {
    ...
    // Collect results
    let mut part_codes: Vec<Option<(String, String, [f64; 3], Vec<[f64; 3]>)>> =
            ...
            Ok((_, Ok((response, part_usage)))) => {
                ...
                match code {
                    Some(c) => {
                        ...
                        part_codes[idx] = Some((name.clone(), c, position, part_spec.instances.clone()));
                        any_success = true;
                        let _ = on_event.send(MultiPartEvent::PartComplete {
                            part_index: idx,
                            part_name: name,
    ...
    let mut accepted_parts: Vec<(String, String, [f64; 3], Vec<[f64; 3]>)> = Vec::new();
    ...
    if let Some(ctx) = execution_ctx {
        for (part_idx, part_entry) in part_codes.iter_mut().enumerate() {
            if let Some((name, code, pos, instances)) = part_entry.clone() {
                ...
                match artifact_result {
                    Ok(artifact) => {
                        ...
                        let instances_for_entry = instances.clone();
                        *part_entry =
                            Some((name.clone(), artifact.code.clone(), pos, instances_for_entry));
                        accepted_parts.push((name, artifact.code, pos, instances));
                    }
                    Err(e) => {
                                ...
                                if let Some(code) = extract_code_from_response(&response) {
                                    ...
                                    .await
                                    {
                                        Ok(artifact) => {
                                            ...
                                            let instances = part_spec.instances.clone();
...
pub async fn generate_parallel(
    ...
    let prompt_tier = prompt_tier_for_bucket(bucket);
...
mod tests {
    ...
    #[test]
    fn parse_plan_accepts_instances() {
        let json = r#"{"mode":"multi","parts":[{"name":"tire","description":"main","position":[0,0,0],"instances":[[10,0,0],[-10,0,0]],"constraints":[]}],"description":"test"}"#;
        let plan = parse_plan(json).expect("plan should parse");
        assert_eq!(plan.parts.len(), 1);
        assert_eq!(plan.parts[0].instances.len(), 2);
        assert_eq!(plan.parts[0].instances[0], [10.0, 0.0, 0.0]);
    }
...
- Create two separate solids/bodies: Housing and BackPlate."#;
    ...
    fn assembly_produces_valid_code() {
        ...
        let mock_parts: Vec<(String, String, [f64; 3], Vec<[f64; 3]>)> = vec![
            (
                "housing".to_string(),
    ...
    fn assembly_places_instances() {
        ...
        let assembled = assemble_parts(&mock_parts).expect("assembly should succeed");
        assert!(assembled.contains("Pos(10, 0, 0) * part_wheel"));
        assert!(assembled.contains("Pos(-10, 0, 0) * part_wheel"));
    }
...
pub async fn retry_skipped_steps(
    ...
    let bucket = context_bucket_for_model(&config.ai_provider, &config.model);
    let prompt_tier = prompt_tier_for_bucket(bucket);
    let mut system_prompt = crate::agent::prompts::build_system_prompt_for_tier(
        config.agent_rules_preset.as_deref(),
        cq_version.as_deref(),
        prompt_tier,
    );
...
pub async fn retry_part(
    ...
    let bucket = context_bucket_for_model(&config.ai_provider, &config.model);
    let prompt_tier = prompt_tier_for_bucket(bucket);
    let mut system_prompt = prompts::build_system_prompt_for_tier(
        config.agent_rules_preset.as_deref(),
        cq_version.as_deref(),
        prompt_tier,
    );
