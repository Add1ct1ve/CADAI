...
fn append_cookbook(prompt: &mut String, rules: &AgentRules, cq_version: Option<&str>) {
    if let Some(ref cookbook) = rules.cookbook {
        ...
        let filtered: Vec<&crate::agent::rules::CookbookEntry> = cookbook
            .iter()
            .filter(|entry| match (&entry.min_version, cq_version) {
...
fn append_design_patterns(prompt: &mut String, rules: &AgentRules) {
...
fn append_anti_patterns(prompt: &mut String, rules: &AgentRules) {
...
fn append_api_reference(prompt: &mut String, rules: &AgentRules) {
...
fn append_dimension_tables(prompt: &mut String, rules: &AgentRules) {
...
fn append_dimension_guidance(prompt: &mut String, rules: &AgentRules) {
...
fn append_failure_prevention(prompt: &mut String, rules: &AgentRules) {
...
fn append_few_shot_examples(prompt: &mut String, rules: &AgentRules) {
...
fn append_response_format(prompt: &mut String, rules: &AgentRules, verbose_fallback: bool) {
...
fn append_output_structure(prompt: &mut String) {
    prompt.push_str("\n## Output Structure\n");
...
pub fn build_system_prompt(rules: &AgentRules, cq_version: Option<&str>) -> String {
    ...
    append_cookbook(&mut prompt, rules, cq_version);
    append_design_patterns(&mut prompt, rules);
    append_anti_patterns(&mut prompt, rules);
    ...
    append_api_reference(&mut prompt, rules);
    append_dimension_tables(&mut prompt, rules);
    append_dimension_guidance(&mut prompt, rules);
    ...
    append_failure_prevention(&mut prompt, rules);
    append_few_shot_examples(&mut prompt, rules);
    append_response_format(&mut prompt, rules, true);
    append_output_structure(&mut prompt);
...
pub fn build_system_prompt_for_tier(
    preset_name: Option<&str>,
    cq_version: Option<&str>,
    tier: PromptTier,
) -> String {
    ...
    match tier {
        PromptTier::Full => build_system_prompt(&rules, cq_version),
        PromptTier::Compact => {
            let mut prompt = build_compact_prompt_core(&rules);
            append_response_format(&mut prompt, &rules, false);
            append_output_structure(&mut prompt);
            prompt
        }
        PromptTier::Expanded => {
...
fn build_compact_prompt_core(rules: &AgentRules) -> String {
    let mut prompt = String::new();
...
mod tests {
    ...
    #[test]
    fn test_prompt_tier_expanded_sections() {
        let prompt = build_system_prompt_for_tier(None, None, PromptTier::Expanded);
        assert!(prompt.contains("## Design Patterns"));
        assert!(prompt.contains("## Common Anti-Patterns"));
        assert!(prompt.contains("## Build123d API Quick-Reference"));
        assert!(prompt.contains("## Failure Prevention"));
    ...
    #[test]
    fn test_prompt_tier_expanded_plus_sections() {
        let prompt = build_system_prompt_for_tier(None, None, PromptTier::ExpandedPlus);
        assert!(prompt.contains("## Design Patterns"));
        assert!(prompt.contains("## Few-Shot Examples"));
        assert!(prompt.contains("## Real-World Dimension Tables"));
        assert!(!prompt.contains("## Build123d Cookbook"));
    ...
    #[test]
    fn test_prompt_tier_full_sections() {
        let prompt = build_system_prompt_for_tier(None, None, PromptTier::Full);
        assert!(prompt.contains("## Build123d Cookbook"));
        assert!(prompt.contains("## Few-Shot Examples"));
    }
}
